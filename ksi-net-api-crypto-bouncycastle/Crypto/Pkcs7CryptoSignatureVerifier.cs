...
namespace Guardtime.KSI.Crypto
{
    ...
    public class Pkcs7CryptoSignatureVerifier : ICryptoSignatureVerifier
    {
        ...
        public void Verify(byte[] signedBytes, byte[] signatureBytes, CryptoSignatureVerificationData data)
        {
            ...
            try
            {
                ...
                if (signerCollection.Count > 1)
                {
                    ...
                    throw new Exception("Signature contains more than one SignerInformation element.");
                }
                ...
                if (signerInfo == null)
                {
                    throw new Exception("Signature does not contain any SignerInformation element.");
                }
                ...
                {
                    throw new Exception("Signature does not contain any x509 certificates.");
                }
                ...
                {
                    throw new Exception("Signer information does not match with certificate.");
                }

                // Verify certificate with selector
                if (!_certificateRdnSelector.Match(certificate))
                {
                    throw new KsiException("Certificate did not match with certificate selector.");
                }
        ...
        private class CertPathChecker : PkixCertPathChecker
        {
            ...
            {
                // TODO: 
                if (unresolvedCritExts.IsEmpty) return;
