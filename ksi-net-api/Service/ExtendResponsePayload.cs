...
namespace Guardtime.KSI.Service
{
    public sealed class ExtendResponsePayload : ExtendPduPayload
    {
        ...
        public ExtendResponsePayload(TlvTag tag) : base(tag)
        {
            ...
            {
                throw new InvalidTlvStructureException("Only one calendar hash chain must exist in extend response payload");
            }

            if (_status.Value != 0 && calendarHashChainCount != 0)
            {
                throw new InvalidTlvStructureException("Calendar hash chain should be missing when error occurs in extend response payload");
            }
        }
